...
try:
    ...
    print("⚠️  Plotly not available. Chart generation will be limited.")
    HAS_PLOTLY = False
...
class AdvancedAnalyticsEngine:
    ...
    def init_analytics_tables(self):
        ...
        try:
            ...
            # Advanced metrics table - FIXED SQL SYNTAX
            cursor.execute('''
            ...
            # Create index separately - FIXED
            cursor.execute('''
                CREATE INDEX IF NOT EXISTS idx_analytics_metrics_group_metric 
                ON analytics_metrics(group_id, metric_type, timeframe)
            ''')
            
            # Engagement tracking - FIXED SQL SYNTAX
            cursor.execute('''
            ...
            # Create unique index separately - FIXED
            cursor.execute('''
                CREATE UNIQUE INDEX IF NOT EXISTS idx_user_engagement_unique 
                ON user_engagement(group_id, user_phone, date)
            ''')
            ...
            # Create unique index for member activity - FIXED
            cursor.execute('''
                CREATE UNIQUE INDEX IF NOT EXISTS idx_member_activity_unique 
                ON member_activity(group_id, phone_number, activity_date)
            ''')
    ...
    def _calculate_engagement_score(self, messages_df: pd.DataFrame) -> float:
        ...
        # Unique participants
        unique_senders = messages_df['sender_phone'].nunique() if 'sender_phone' in messages_df.columns else 1
        
        # Message distribution among users
        if 'sender_phone' in messages_df.columns:
            user_messages = messages_df['sender_phone'].value_counts()
        else:
            # Fallback if sender_phone column doesn't exist
            user_messages = messages_df.get('sender', messages_df.get('from', pd.Series([1]))).value_counts()
    ...
    def _analyze_engagement_trend(self, messages_df: pd.DataFrame) -> Dict:
        ...
        sender_col = 'sender_phone' if 'sender_phone' in messages_df.columns else 'sender'
        if sender_col not in messages_df.columns:
            return {'trend': 'no_sender_data'}
        ...
        daily_msg_per_user = messages_df.groupby([
            messages_df['timestamp'].dt.date, sender_col
    ...
    def _calculate_group_metrics(self, group_id: str, timeframe_days: int) -> Dict:
        ...
        metrics = {
            'total_messages': len(messages_df),
            'unique_senders': messages_df[sender_col].nunique() if sender_col in messages_df.columns else 1,
    ...
    def _calculate_platform_metrics(self, timeframe_days: int) -> Dict:
        ...
        metrics = {
            'total_messages': len(messages_df),
            'total_groups': messages_df['group_id'].nunique() if 'group_id' in messages_df.columns else 1,
            'active_groups': messages_df['group_id'].nunique() if 'group_id' in messages_df.columns else 1,
            'total_users': messages_df[sender_col].nunique() if sender_col in messages_df.columns else 1,
            'avg_group_activity': 0,
            'flagged_messages': messages_df['is_flagged'].sum() if 'is_flagged' in messages_df.columns else 0
        }
        
        # Calculate average group activity safely
        if 'group_id' in messages_df.columns and messages_df['group_id'].nunique() > 0:
            metrics['avg_group_activity'] = len(messages_df) / messages_df['group_id'].nunique()
    ...
    def _generate_chart_data(self, group_id: str = None, timeframe_days: int = 30) -> Dict:
        ...
        sender_col = 'sender_phone' if 'sender_phone' in messages_df.columns else 'sender'
        if sender_col in messages_df.columns:
            user_messages = messages_df[sender_col].value_counts().head(10)
    ...
    def generate_plotly_charts(self, group_id: str = None, timeframe_days: int = 30) -> Dict:
        ...
        if not HAS_PLOTLY:
            return {'error': 'Plotly not available. Please install: pip install plotly'}
